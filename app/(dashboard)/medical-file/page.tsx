...
// نوع record مش لازم نحط generics في maybeSingle
...
export default function PregnancyPage() {
  ...
  const fetchRecord = async () => {
    ...
    if (user) {
      ...
      if (data) {
        const rec = data as PregnancyRecord;
        setRecord(rec);
        setLastPeriod(rec.last_period_date);
      }
    }
  };
        ...
        {record ? (
          <div>
            <h2 className="text-3xl font-bold text-gray-800 mb-1">
              الأسبوع {calculateWeek(lastPeriod)}
            </h2>
            <p className="text-gray-500 text-sm">
              أنتِ الآن في الثلث {calculateWeek(lastPeriod) < 13
                ? 'الأول'
                : calculateWeek(lastPeriod) < 27
                ? 'الثاني'
                : 'الثالث'}
            </p>
